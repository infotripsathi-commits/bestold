...
import { Plus, Pencil, Trash2, Car, GripVertical } from 'lucide-react';
import { getAllCarBrands, createCarBrand, updateCarBrand, deleteCarBrand } from '@/db/api';
...
export default function AdminCarBrandsPage() {
  ...
  const loadBrands = async () => {
    try {
      const data = await getAllCarBrands();
      setBrands(data);
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingBrand) {
        await updateCarBrand(editingBrand.id, formData);
        toast.success('Brand updated');
      } else {
        await createCarBrand(formData);
  ...
  const handleToggleActive = async (brand: CarBrand) => {
    try {
      await updateCarBrand(brand.id, { is_active: !brand.is_active });
    ...
    <div className="container py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Manage car brands available for sellers to select when listing a car.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">All Car Brands</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>No car brands yet. Add your first brand.</p>
            </div>
          ) : (
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent className="max-w-[calc(100%-2rem)] md:max-w-lg">
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This will permanently delete this car brand. Existing listings that used this brand will keep their stored value.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingBrand ? 'Update brand details.' : 'Add a new car brand for sellers to choose from.'}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="flex items-center justify-between rounded-lg border border-border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Inactive brands won't appear in seller forms.</p>
              </div>
              <Switch
                id="brand-active"
